...
def cmake_version():
    cmake_version_raw = subprocess.check_output(
        ["cmake", "--version"]
    ).decode()
    cmake_version_match = next(
        re.finditer(
...
def simple_cxx_project(build_tree):
    (build_tree.source / "CMakeLists.txt").write_text(
        textwrap.dedent(
            """\
    ...
    (build_tree.source / "alib.cpp").write_text(
        textwrap.dedent(
            r"""\
        #include <iostream>
        #include <string>
...
def complex_cxx_project(build_tree):
    ...
    (build_tree.source / "lib1.cpp").write_text(
        textwrap.dedent(
            r"""\
        #include <iostream>
        #include <string>
    ...
    (build_tree.source / "lib2.cpp").write_text(
        textwrap.dedent(
            r"""\
        #include <iostream>
        #include <string>
    ...
    (build_tree.source / "exe1.cpp").write_text(
        textwrap.dedent(
            r"""\
        #include <iostream>
        int main() {
    ...
    (build_tree.source / "exe2.cpp").write_text(
        textwrap.dedent(
            r"""\
        #include <string>
        void print_something(const std::string &s);
    ...
    (build_tree.source / "exe3.cpp").write_text(
        textwrap.dedent(
            r"""\
        #include <string>
        void print_extra(const std::string &s);
...
def test_codemodelV2(simple_cxx_project, capsys):
    project = CMakeProject(
        simple_cxx_project.build, simple_cxx_project.source, api_version=1
    )
...
def test_complete_project(complex_cxx_project, capsys):
    project = CMakeProject(
        complex_cxx_project.build, complex_cxx_project.source, api_version=1
    )
...
@pytest.mark.skipif(
    not CMAKE_SUPPORTS_TOOLCHAINS_V1,
    reason="CMake does not support toolchains V1 kind",
)
def test_toolchain_kind_cxx(complex_cxx_project, capsys):
    ...
    assert "CXX" in tuple(
        toolchain.language for toolchain in kind_obj.toolchains
    )
